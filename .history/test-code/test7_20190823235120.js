...
for (i = 0; i < levelArray.length; i++) {
    ...
    console.log(pos + ' - ' + circleWithEnharmonics[pos].keyinfo['signature']);

    console.log(circleWithEnharmonics[pos]);
    var sharps = circleWithEnharmonics[pos].keyInfo.signature.hasOwnProperty('sharps') ? circleWithEnharmonics[pos].keyinfo.signature.sharps : 0;
    var flats = circleWithEnharmonics[pos].keyInfo.signature.hasOwnProperty('flats') ? circleWithEnharmonics[pos].keyinfo['signature'] : 0;

    console.log(pos + '/' + sharps + '/' + flats);
    //const Q = Math.floor(Math.random() * 2); // zero or 1
    const Q = 0;
    let QUESTION = keySignatureQuestions[quality][Q].replace(/\[key\]/g, keyToFind); // replace placeholder with the key
