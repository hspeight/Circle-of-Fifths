...
console.log(resolutionsPerAuthority.filter(record => (record.authority === "amzn1.er-authority.echo-sdk.amzn1.ask.skill.99cfc3e2-7eea-4c93-a953-c33d0527a941.packType")));


function getNextPurchaseableProduct() {

    for (var j = 0; j < resolutionsPerAuthority.length; j++) {
        if (resolutionsPerAuthority[j].values.value === 'relative-keys') {
