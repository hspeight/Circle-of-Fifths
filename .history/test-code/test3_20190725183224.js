...
var circleData = [
    ...
    {'key': 'c flat', 'relmaj': 'c flat/b', 'relmin:': 'g sharp/a flat', 'data': ['e','b/c flat','0','7']},
...
for (i = 0; i < roundArray.length; i++) {
    ...
    if (relation === 'minor') {
        ...
        console.log('pos=' + pos + 'relative ' + relation + ' of ' + keyToFind + ' minor is ' + circleData[pos]['relmaj']);
        //relMaj = circleData[pos]['key'];
        //relMin = circleData[pos]['relmin'];
    }
    relMin = circleData[pos]['relmin'];
    relMaj = circleData[pos]['relmaj'];
    console.log('##' + 'rel maj=' +  relMaj + ' rel min=' + relMin);
    var s = circleData[pos]['relmaj'].split().concat(circleData[pos]['data'].concat(circleData[pos]['relmin']));
    console.log('####' + s);
