...
const AnswerHandler = {
    ...
    handle(handlerInput) {
        ...
        if (supportsAPL(handlerInput)) {
            dataNextQuestion.bodyTemplate7Data.text.drill = drills[attributes.currentDrillIndex].packName;
        dataNextQuestion.bodyTemplate7Data.text.level = requestAttributes.t('LEVEL_TXT', attributes.drillStatus[attributes.drillStatusIndex].drill.level,
                                                drills[attributes.currentDrillIndex]['levels'].length, attributes.questionNum);
            console.log('attributes.skillState === ' + attributes.skillState);
            switch (attributes.skillState) {
                case states.REPLAYLEVEL:
                    dataNextQuestion.bodyTemplate7Data.text.level = requestAttributes.t('ASP_END_OF_LEVEL', attributes.drillStatus[attributes.drillStatusIndex].drill.level,
                                                                        drills[attributes.currentDrillIndex]['levels'].length);
                    ...
                    case states.ENDOFDRILLWITHUPSELL:
                        dataNextQuestion.bodyTemplate7Data.text.level = requestAttributes.t('ASP_END_OF_LEVEL', attributes.drillStatus[attributes.drillStatusIndex].drill.level,
                                                                            drills[attributes.currentDrillIndex]['levels'].length);
                        dataNextQuestion.bodyTemplate7Data.text.question = requestAttributes.t('ASP_END_OF_DRILL', attributes.currentDrillName);
                        break;
                default:
        ...
        switch (attributes.skillState) {
            ...
            case states.REPLAYLEVEL:
                //speechOutput = endOfLevel(handlerInput) + qta;
                    ...
                    speechOutput = requestAttributes.t('START_LEVEL_MSG', attributes.currentDrillName, attributes.drillStatus[attributes.drillStatusIndex].drill.level) +
            ...
            case states.ENDOFDRILLWITHREPLAY:
                ...
                attributes.drillStatus[attributes.drillStatusIndex].drill.level = 0;
...
*/
    ...
    if (attributes.launchCount === 1) {
        //attributes.drillStatus[attributes.drillStatusIndex].drill.level = 0;
        intro += requestAttributes.t('FIRST_TIME', drills[0].packName, drills[0].levels.length); // e.g. perfect intervals, 12 levels;
    } else {
        intro += requestAttributes.t('WELCOME_BACK', drills[attributes.currentDrillIndex].attributes.drillName,
        attributes.drillStatus[attributes.drillStatusIndex].drill.level) + requestAttributes.t('ASK_IF_READY');
    }
    if (attributes.skillState != states.REPLAYLEVEL) {
        attributes.drillStatus[attributes.drillStatusIndex].drill.level += 1; //wrong wrong wrong or is it?
    }
...
function marksOutOfTen(handlerInput) {
    ...
    if (percentScore > 99) {
        ...
        if (attributes.drillStatus[attributes.drillStatusIndex].drill.level === drillLevels) { // Have all rounds in this drill been completed?
            ...
            speechOutput = requestAttributes.t('END_OF_DRILL', attributes.currentDrillName, attributes.nextProd.referenceName);
            repromptOutput = speechOutput;
            ...
            speechOutput = requestAttributes.t('END_LEVEL_MESSAGE_1', attributes.currentDrillName, attributes.drillStatus[attributes.drillStatusIndex].drill.level,
        ...
        speechOutput = requestAttributes.t('ASK_REPLAY_LEVEL', attributes.currentDrillName, attributes.drillStatus[0].drill.level, attributes.levelScore, QUESTIONS.length);
                ...
                handlerInput.attributesManager.getPersistentAttributes()
                    .then((persistentAttributes) => {
                        ...
                        if (!persistentAttributes.launchCount) { // first ever launch
                            //console.log('SETTING LAUNCHCOUNT TO ZERO');
