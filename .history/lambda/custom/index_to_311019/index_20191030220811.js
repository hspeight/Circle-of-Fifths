    ...
    handle(handlerInput) {
        ...
        switch (attributes.skillState) {
            case states.PACKDONE:
                ...
                speechOutput = 'Ok. What would you like to do? You can say what can I play, or what can I buy.';
                repromptOutput = 'What would you like to do? You can say what can I play, or what can I buy.';
                break;
            case states.QUIZ:
                    speechOutput = 'Ok. What would you like to do? You can say what can I play, or what can I buy.';
                    repromptOutput = 'What would you like to do? You can say what can I play, or what can I buy.';
            ...
            case states.ENDOFDRILLWITHUPSELL:
                speechOutput = 'Ok. you can yada yada'; // convert to RA
                repromptOutput = 'what to do next?';
                break;
            default:
                speechOutput = "Sorry i don't understand that! " + attributes.currentQTA; // attention
                break;
        }
    ...
    async handle(handlerInput) {
        ...
        switch (attributes.skillState) {
            ...
            case states.ENDOFLEVEL:
                return setupLevel(handlerInput);
            case states.ENDOFDRILLWITHUPSELL:
                return makeUpsell('OK.', handlerInput); // offer the first available purchaseable product. whhat if there are none left to buy???7    
            default:
...
function getNextQuestion(handlerInput) {
    ...
    if (attributes.questionNum < QUESTIONS.length) {
        ...
        if (attributes.currentLevel === attributes.drillLevels) { // Have all rounds in this drill been completed?
            ...
            retval = requestAttributes.t('END_OF_DRILL', attributes.drillName, attributes.nextProd.name);
        } else {
...
function makeUpsell(preUpsellMessage, handlerInput) {

    const attributes = handlerInput.attributesManager.getSessionAttributes();
    const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
    let upsellMessage = `${preUpsellMessage} ${attributes.nextProd.summary}. ${requestAttributes.t('RANDOM_LEARN_MORE_PROMPT', attributes.nextProd.name)}`;
    ...
    return handlerInput.responseBuilder
        .addDirective({
            ...
            payload: {
                InSkillProduct: {
                    productId: attributes.nextProd.productId
                },
...
const PurchaseHistoryHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (attributes.entitledProducts.length > 0) { // please convert to requestattributes
            ...
            speechOutput = requestAttributes.t('NO_PRODUCTS_OWNED') + requestAttributes.t('WHAT_CAN_I_PLAY_OR_BUY');
            repromptOutput = requestAttributes.t('WHAT_CAN_I_PLAY_OR_BUY');
        }
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (ISPHelp.isEntitled(attributes.entitledProducts, packRef)) {
                ...
                attributes.nextProd = getNextPurchasableProduct(handlerInput);
                return makeUpsell('You don\'t currently own the ' + packRef + ' pack.', handlerInput);
            }
        });
    }
        ...
        return ms.getInSkillProducts(locale).then(async function handlePurchaseResponse(res) {
            ...
            if (handlerInput.requestEnvelope.request.status.code === '200') {
                ...
                switch (handlerInput.requestEnvelope.request.payload.purchaseResult) {
                    case 'ACCEPTED':
                        // use the product ref to get the product name 
