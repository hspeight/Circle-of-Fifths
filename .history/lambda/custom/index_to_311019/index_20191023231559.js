...
function askQuestion(handlerInput) {
    ...
    attributes.currentQTA = `Question ${attributes.questionNum}. ${QUESTIONS[attributes.questionNum -1]}`;
...
function setupLevel(handlerInput) {
    ...
    _.forIn(drills, (value, key) => {
        ...
        if (key === attributes.currentDrillRef) {
            ...
            console.log(';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;' + JSON.stringify(value.levels[0]));
            console.log(';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;******************' + attributes.currentLevel);
            //console.log(value.levels[0]);
            //attributes.level = value.levels[attributes.currentLevel -1];
            attributes.level = value.levels[attributes.currentLevel - 1];
            console.log(';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;+++++++++++++++++++' + JSON.stringify(attributes.level));
            console.log(';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;+++++++++++++++++++' + attributes.currentLevel);
...
*/
    ...
    switch (attributes.currentDrillRef) {
        ...
        console.log(';;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;============;' + JSON.stringify(attributes.level));
            QANDA = drillSetup.perfectIntervals(attributes, userId);
            break;
        case 'relative-keys':
            QANDA = drillSetup.relativeKeys(attributes, userId);
            break;
        case 'key-signatures':
            QANDA = drillSetup.keySignatures(attributes, userId);
            break;
        default:
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (ISPHelp.isEntitled(attributes.entitledProducts, packRef)) {
                ...
                //console.log(qta);
                //let qta = 'hector '; // so why don't the qta above return something sensible?
                // Give the user a 'drill completed' message here if the current drill has been completed
                //speechOutput = requestAttributes.t('START_LEVEL_MSG', attributes.drillName, attributes.currentLevel) + qta;
                //repromptOutput = qta;
            ...
            if (handlerInput.requestEnvelope.request.status.code === '200') {
                ...
                switch (handlerInput.requestEnvelope.request.payload.purchaseResult) {
                    case 'ACCEPTED':
                        ...
                        attributes.drillName = newProd[0].name;
                        console.log('referenceName of newly purchased product:');
                        console.log(newProd[0]);
                        attributes.currentLevel = 1;
                        attributes.skillState = states.STARTLEVEL;
                        return setupLevel(handlerInput); // requires attention
                        //console.log('need questions here');
                        //console.log(QUESTIONS);
                        //speechOutput = requestAttributes.t('STARTER2', attributes.drillName + ', ', 1 + ', ') + askQuestion();
                        //break;
                ...
                handlerInput.attributesManager.getPersistentAttributes()
                    .then((persistentAttributes) => {
                        ...
                        if (!persistentAttributes.launchCount) { // first ever launch
                            ...
                            obj.level = 1;
                            obj.completed = false;
                            persistentAttributes.drillStatus.push(JSON.parse(JSON.stringify(obj))); //persistentAttributes.currentLevel = 1;
                        ...
                        persistentAttributes.levelData = getLevelData(persistentAttributes.currentDrillRef);
                        persistentAttributes.launchCount += 1;
                        console.log('GET ######################################' + persistentAttributes.launchCount, persistentAttributes.currentLevel);
...
function getLevelData(drillref, level) {
    ...
    let arr = Object.keys(drills);
    let DRILLREF = arr[arr.indexOf(drillref)]; //e.g. key-signatures
    return drills[DRILLREF].levels[level].data;
}
