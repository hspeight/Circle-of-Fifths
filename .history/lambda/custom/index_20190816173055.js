...
const BuyPackIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        speechOutput = 'Hello here i am';
        repromptOutput = 'please don\'t end the session';
            return handlerInput.responseBuilder
                .speak(speechOutput)
                .reprompt(repromptOutput)
                .getResponse();

        //check the requested product against the list of purchaseable products
        /*
        let prodId = undefined;
        for (let i = 0; i < attributes.purchasableProducts.length; i++) {
            if (attributes.purchasableProducts[i].name.toLocaleLowerCase() === handlerInput.requestEnvelope.request.intent.slots.potty.value.toLocaleLowerCase()) {
                prodId = attributes.purchasableProducts[i].productId;
                break;
            }
        }
        */
       
       //console.log('handlerInput.requestEnvelope.request.intent.slots.potty.value=' + handlerInput.requestEnvelope.request.intent.slots.potty.value);
       /*
        if (typeof prodId === 'undefined') {
            console.log('purchaseable list length=' + attributes.purchasableProducts.length);

            // show purchaseable packs on devices with screeens
            if (attributes.purchasableProducts.length === 0) {
                speechOutput = `There are no products available to purchase. You can say what have I bought to get a list of your purchased products.`;
            } else if (attributes.purchasableProducts.length === 1) {
                speechOutput = `Products available for purchase are, ${ISPHelp.getSpeakableListOfProducts(attributes.purchasableProducts)}. Would you like to buy it?`;
            } else {
                speechOutput = `Products available for purchase are, ${ISPHelp.getSpeakableListOfProducts(attributes.purchasableProducts)}. Which one would you like to buy?`;
            }
            repromptOutput = 'please don\'t end the session';
            return handlerInput.responseBuilder
                .speak(speechOutput)
