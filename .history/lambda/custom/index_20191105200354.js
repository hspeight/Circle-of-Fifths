    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (ISPHelp.isEntitled(attributes.entitledProducts, packRef)) {
                ...
                let productToUpsell = ISPHelp.getPurchasableProduct(attributes.purchasableProducts, handlerInput.requestEnvelope.request.payload.productId);
                console.log('################################');
                console.log(productToUpsell);
                console.log('################################');
                //attributes.nextProd = ISPHelp.getNextPurchasableProduct(attributes.purchasableProducts);
                return makeUpsell('You don\'t currently own the ' + productToUpsell.name + ' pack.', productToUpsell, handlerInput);
            }
        });
    }
