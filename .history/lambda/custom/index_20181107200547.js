...
const validQuiz = ['fourths', 'fifths', 'mixed', '4ths', '4th', '5ths', '5th', 'pick', 'any', 'choose', 'either', 'mix', 'mixture'];
var utteredAnswer = '';
//var itemList = [];
...
//var relQuestionsMix = relQuestions4th.concat(relQuestions5th);
var relAnswers = [];
...
const LaunchRequestHandler = {
    ...
    handle(handlerInput) {
        ...
        var welcomePrefix = 'Good ' + util.periodOfDay(date.getHours()) + '! and ';
...
const QuizAnswerHandler = {
    ...
    handle(handlerInput) {
        ...
        if (attributes.skillState === states.END) {
            return response.speak(speechOutput)
            .reprompt(repromptOutput)
            .withSimpleCard(skillName + ' - test summary', printCardInfo(handlerInput)) // print list of QandA in the form 'What is D the fourth of?' You: A, Ans: A
            .getResponse();
        } else {
            return response.speak(speechOutput)
            .reprompt(repromptOutput)
...
function HandleTheAnswer(handlerInput, answerIn) {
    ...
    if (utteredAnswer < 1 || utteredAnswer > languageStrings.en.translation.CHOICES) {
        ...
        //formatQuestion(attributes.counter, questionArray[attributes.counter -1],
        //                    multiChoiceArray[utteredAnswer -1], relAnswers[attributes.counter -1]); //NEEDS WORK!!!!!!
...
function exitSkill(pHandlerInput) {
  ...
  //console.log('exitskil attributes.skillState=' + attributes.skillState);  
    if (attributes.skillState === states.QUIZ) {
        ...
        //.withSimpleCard(skillName + ' - early end', printCardInfo())
        //.getResponse();
    //} else {
        return response
        .speak(requestAttributes.t('EXIT_SKILL_MESSAGE'))
...
function getSpeechCon(handlerInput, type) {
    const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
    return type ? requestAttributes.t('CORRECT_ANSWER') : '<audio src=' + asset.wrongAnswerAudio + '/>';
}
