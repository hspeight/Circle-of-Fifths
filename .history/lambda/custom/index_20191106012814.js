...
const AnswerHandler = {
    ...
    handle(handlerInput) {
        ...
        attributes.skillState = attributes.QUIZ;
...
function askAQuestion(handlerInput) {
    ...
    attributes.skillState = attributes.AWAITINGANSWER;
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (ISPHelp.isEntitled(attributes.entitledProducts, packRef)) {
                ...
                return makeUpsell('You don\'t currently own the ' + attributes.nextProd[0].name + ' pack.', attributes.nextProd[0], handlerInput);
            }
        });
    }
    ...
    handle(handlerInput) {
        ...
        switch (attributes.skillState) {
            case states.AWAITINGANSWER:
                    speechOutput = requestAttributes.t('NO_COMPRENDE') + 'Question ' + attributes.questionNum + ' ' + QUESTIONS[attributes.questionNum -1];
                    repromptOutput = 'Question ' + attributes.questionNum + ' ' + QUESTIONS[attributes.questionNum -1];
            default:
                speechOutput = requestAttributes.t('NO_COMPRENDE') + requestAttributes.t('WHAT_CAN_I_PLAY_OR_BUY_REPROMPT');
                repromptOutput = requestAttributes.t('WHAT_CAN_I_PLAY_OR_BUY_REPROMPT');
                break;
