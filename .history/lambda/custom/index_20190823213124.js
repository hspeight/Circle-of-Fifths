...
function CheckAnswer(handlerInput, answerIn) {
    ...
    var enharmonic = ANSWERS[attributes.questionNum - 1].split('/');
    //var TOF = '';
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (product.length === 0) { // User has aked for the free pack
                let qta = setupLevel(handlerInput);
                attributes.drillRef = util.getPackName(drills, drills[0].id)[0].packName;
                //pass drill name and level to string
                speechOutput = requestAttributes.t('CURRENT_LEVEL', attributes.drillRef, attributes.drillStatus[0].drill.level) + qta;
                repromptOutput = qta;
