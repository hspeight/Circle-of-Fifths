    ...
    async handle(handlerInput) {
        ...
        switch (attributes.skillState) {
            ...
            case states.ENDOFDRILL:
                ...
                return makeUpsell('OK.', attributes.nextDrillRef, handlerInput); // offer the first available purchaseable product. whhat if there are none left to buy???
...
function marksOutOfTen(handlerInput) {
    ...
    if (percentScore > 99) {
        ...
        if (attributes.drillStatus[0].drill.level === drillLevels) { // Have all rounds in this drill been completed?
            ...
            attributes.nextDrillRef = getNextPurchasableProduct(attributes.purchasableProducts);
            speech.speech = requestAttributes.t('END_OF_DRILL', attributes.thisDrillName, attributes.nextDrillRef);
            speech.reprompt = speech.speech;
