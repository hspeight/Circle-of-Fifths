...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (attributes.launchCount == 1) {
            ...
            speechOutput = requestAttributes.t('WELCOME_BACK', attributes.thisDrillName, attributes.drillStatus[0].drill.level) +
        ...
        switch (attributes.skillState) {
            ...
            case states.REPLAY:
                ...
                speechOutput = requestAttributes.t('START_LEVEL_MSG', attributes.thisDrillName, attributes.drillStatus[0].drill.level) +
                    ...
                    setupLevel(handlerInput, states.ENDOFLEVEL);
                break;
            case states.ENDOFDRILL:
                //speechOutput = endOfDrill(handlerInput) + qta;
                //return makeUpsell('OK', attributes.purchasableProducts, handlerInput);
                return makeUpsell('OK.', attributes.purchasableProducts[0], handlerInput); // offer the first available purchaseable product. whhat if there are none left to buy???
...
function setupLevel(handlerInput, STATE_IN) {
    console.log('setupLevel*********************************************');
    //const attributesManager = handlerInput.attributesManager;
    const attributes = handlerInput.attributesManager.getSessionAttributes();

    console.log('attributes.drill=' + attributes.drill + ' attributes.drillStatus[0].drill.level=' + attributes.drillStatus[0].drill.level);

    var roundData = drills[attributes.drill - 1]['levels'][attributes.drillStatus[0].drill.level - 1]; // drill/round
    ...
    attributes.roundLength = drills[attributes.drill - 1]['levels'][attributes.drillStatus[0].drill.level - 1]['data'].length; // Number of questions in the round
    attributes.thisDrillName = drills[attributes.drill - 1]['packName'];
    attributes.nextDrillName = attributes.purchasableProducts[0].name; // pick the next available pack as the next drill
...
function marksOutOfTen(handlerInput) {
    ...
    if (percentScore > 99) {
        ...
        console.log('attributes.drillStatus[0].drill.level ' + attributes.drillStatus[0].drill.level + '/' + attributes.drill + '/' + attributes.drillLevels);
        if (attributes.drillStatus[0].drill.level === attributes.drillLevels) { // Have all rounds in this drill been completed?
            ...
            speech.speech = requestAttributes.t('END_OF_DRILL', attributes.thisDrillName, attributes.nextDrillName);
            speech.reprompt = speech.speech;
            attributes.skillState = states.ENDOFDRILL; // time to upsell
        } else {
            speech.speech = requestAttributes.t('END_LEVEL_MESSAGE_1', attributes.thisDrillName, attributes.drill, attributes.levelScore, QUESTIONS.length) +
...
//what if user gets here by bypassing launchrequest on first ever invocation?
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            //return makeUpsell('Sure.', attributes.purchasableProducts, handlerInput);
            ...
            if (product.length == 0) { // User has aked for the free pack
                var qta = setupLevel(handlerInput, states.START);
                //let drillObj = drills.filter(function(drill){ return drill.id === "relative-keys" });
                speechOutput = requestAttributes.t('CURRENT_LEVEL', attributes.drillStatus[0].drill.name, attributes.drillStatus[0].drill.level) + qta;
                // need to make sure the questions start from number 1 at this level.
                return handlerInput.responseBuilder
                .speak(speechOutput)
        ...
        return ms.getInSkillProducts(locale).then(function (res) {
            ...
            return handlerInput.responseBuilder
                .addDirective({
                    ...
                    'payload': {
                        'InSkillProduct': {
                            'productId': product[0].productId // element 0 should be safe as there should only ever be one product returned. WRONG!! can have 2 vals like this
                        }
                    },
                    'token': 'correlationToken'
                })
    ...
    handle(handlerInput) {
        ...
        return ms.getInSkillProducts(locale).then(async function handlePurchaseResponse(res) {
            ...
            if (handlerInput.requestEnvelope.request.status.code === '200') {
                ...
                console.log('{{{{{{{{}}}}}}}}}}' + handlerInput.requestEnvelope.request.payload.productId);
                ...
                switch (handlerInput.requestEnvelope.request.payload.purchaseResult) {
                    case 'ACCEPTED':
                        ...
                        var qta = setupLevel(handlerInput, states.START); // requires attention
...
const PersistenceResponseInterceptor = {
    process(handlerInput, response) {
        ...
        console.log(r);
        console.log('----------------------');
        console.log(sessionAttributes.level);
        console.log('........................................................................................');
