...
const skillName = 'Circle Of Fifths'; // surely should be in en.js
...
//***************************************************************************** */
// need to do this each time a new test is started
// get the 12 keys which will include three random enharmonics
var circleKeys = util.makeACircle(circleKeys_);
...
//console.log('starting with fifteenKeys=' + fifteenKeys);
//***************************************************************************** */
//var numQuestions = circleKeys.length; // make this 6 for lite version and up to 12 for pro.
...
// construct the list of keys that can be used for the multi choice questions
...
//console.log('AAAAAAAAAAAAA');
        ...
        //console.log('numQuestionsThisUser=' + attributes.numQuestionsThisUser);
...
const SetNumberOfQuestionsHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (NUM < 1 || NUM > circleKeys.length) {
            speechOutput = requestAttributes.t('OUT_OF_RANGE_MSG', circleKeys.length);
            repromptOutput = requestAttributes.t('OUT_OF_RANGE_MSG', circleKeys.length);
        } else {
            const dbattributes = await handlerInput.attributesManager.getPersistentAttributes(); // <--dynamodb
            dbattributes.questionsPerTest = NUM;
            handlerInput.attributesManager.setPersistentAttributes(dbattributes);
            await handlerInput.attributesManager.savePersistentAttributes();
            speechOutput = requestAttributes.t('NUM_QUESTIONS_CHANGED', NUM);
...
async function exitSkill(handlerInput) {
    const response = handlerInput.responseBuilder;
    const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
    const attributes = handlerInput.attributesManager.getSessionAttributes();
  ...
  //console.log('exitskil attributes.skillState=' + attributes.skillState);  
    if (attributes.skillState === states.QUIZ) {
        response.withSimpleCard(skillName + ' - early end!', printCardInfo(handlerInput)); // needs attention
    }
...
async function setupQuiz(handlerInput, quiz) {
    ...
    // Get the number of questions per test from dynamodb
    const dbattributes = await handlerInput.attributesManager.getPersistentAttributes(); // <--dynamodb
    numQuestions = dbattributes.questionsPerTest;
...
const ErrorHandler = {
    ...
    handle(handlerInput, error) {
        //const E = `Error handled: ${error.message}`;
        //if (E.startsWith('Error handled: Could not read item')) {
...
function bodyTemplateMaker(pBodyTemplateType, handlerInput, pImg, pTitle, pText1, pText2, pText3, pOutputSpeech, pReprompt, pHint, pBackgroundIMG, pEndSession) {
    ...
    if (attributes.skillState === states.END) // Not working for touch response. Maybe an Alexa bug as the card info shows in the reponse json
        response.withSimpleCard(skillName + ' - test summary?', printCardInfo(handlerInput));
