...
exports.pickEnharnomics = function (arrayIn) {
  ...
  for (i = 0, len = arrayIn.length; i < len; i++) {
    ...
    for (j = 1, len2 = arrayIn[i].length; j < 3; j++) {
      //console.log('!' + arrayIn[i][j] + '!');
      a = arrayIn[i][j].split('/');
      //var enhKey = a.length == 2 ? a[Math.floor((Math.random() * 2))] : 0; // random number between 0 and 1 or 0
      if (a.length == 2) {
        var enhKey = a[Math.floor((Math.random() * 2))]; // random number between 0 and 1
      } else {
        var enhKey = a[0];
      }
      arrayOut[i][j].push(enhKey);

      //var key = a.length == 2 ? a[Math.floor((Math.random() * 2))] : a[0];
      console.log('!' + a + '!' + a.length + '!' + arrayOut[i][j]);
