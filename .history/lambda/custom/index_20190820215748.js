...
function setupLevel(handlerInput, STATE_IN) {
    ...
    // line below is probably WRONG!! seach should be based on id NOT element.
    // that would allow the newly purchased pack to be found and set up here
    ...
    var roundData = drills[attributes.currentDrill - 1]['levels'][attributes.currentlevel - 1]; // drill/round
    ...
    drillLevels = drills[attributes.currentDrill - 1]['levels'].length;
    attributes.roundLength = drills[attributes.currentDrill - 1]['levels'][attributes.currentlevel - 1]['data'].length; // Number of questions in the round
    attributes.thisDrillName = drills[attributes.currentDrill - 1]['packName'];
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (ISPHelp.isEntitled(product)) {
                let qta = setupLevel(handlerInput, states.START);
            ...
            if (handlerInput.requestEnvelope.request.status.code === '200') {
                ...
                switch (handlerInput.requestEnvelope.request.payload.purchaseResult) {
                    case 'ACCEPTED':
                        ...
                        attributes.drillId = attributes.entitledProducts.filter(function (drill) {
                                return drill.productId.referenceName === handlerInput.requestEnvelope.request.payload.productId});
                                console.log('referenceName of newly purchased product=' + attributes.drillId);
                ...
                handlerInput.attributesManager.getPersistentAttributes()
                    .then((persistentAttributes) => {
                        ...
                        if (!persistentAttributes.launchCount) { // first ever launch
                            ...
                            persistentAttributes.drillId = 'perfect-intervals'; // all new ussers start with the free perfect intervals drill
