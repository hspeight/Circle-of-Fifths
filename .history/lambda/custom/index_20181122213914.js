...
function exitSkill(handlerInput) {
    ...
    if (attributes.skillState === states.QUIZ) { // <-- need a bodytemplate if hasdisplay
        if (supportsDisplay(handlerInput)) {
            response.withSimpleCard(requestAttributes.t('SKILL_NAME'), 'Display text here');
            return bodyTemplateMaker('BodyTemplate6', handlerInput, null, null, 'body templaye 6 text', null, null, 
                null, null, null, asset.ASSET_IMG.mainImgBlurBG, false);
        //    console.log('exitSkill and supportsDisplay');
        } else {
