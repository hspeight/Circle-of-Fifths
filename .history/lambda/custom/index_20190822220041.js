...
function marksOutOfTen(handlerInput) {
    ...
    if (percentScore > 99) {
        ...
        if (attributes.drillStatus[0].drill.level === drillLevels) { // Have all rounds in this drill been completed?
            ...
            attributes.nextDrillName = getNextPurchasableProduct();
...
function getNextPurchasableProduct() {
    ...
    for (var i = 1; i < drills.length; i++) {  // no need to start with product 0 as that is free
        var h = attributes.purchasableProducts.filter(function (p) {
    ...
    return i < drills.length ? h[0].referenceName : 'No packs available for purchase';
}
