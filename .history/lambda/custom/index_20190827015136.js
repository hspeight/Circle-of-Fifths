...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (attributes.launchCount === 1) {
            ...
            speechOutput = requestAttributes.t('WELCOME_BACK', drills[attributes.currentDrillIndex].packName, attributes.currentLevel) +
...
function setupLevel(handlerInput) {
    ...
    if (attributes.drillStatus[attributes.drillStatusIndex].completed) {
...
function marksOutOfTen(handlerInput) {
    ...
    if (percentScore > 99) {
        ...
        if (attributes.drillStatus[0].drill.level === drillLevels) { // Have all rounds in this drill been completed?
            ...
            attributes.drillStatus.drill.completed = true;
        } else {
            speech.speech = requestAttributes.t('END_LEVEL_MESSAGE_1', attributes.currentDrillName, attributes.currentLevel, attributes.levelScore, QUESTIONS.length) +
            ...
            attributes.drillStatus[0].drill.level += 1; //wrong wrong wrong
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (product.length === 0) { // User has aked for the free pack
                ...
                speechOutput = requestAttributes.t('CURRENT_LEVEL', drills[0].packName, attributes.currentLevel) + qta;
                repromptOutput = qta;
                return handlerInput.responseBuilder
                    .speak(speechOutput)
