...
function marksOutOfTen(handlerInput) {
    ...
    if (percentScore > 99) {
        ...
        speech.speech = requestAttributes.t('ASK_PLAY_AGAIN', attributes.level, attributes.levelScore, QUESTIONS.length);
        speech.reprompt = speech.speech;
...
const PersistenceResponseInterceptor = {
    process(handlerInput, responseOutput) {
        ...
        if ((handlerInput.requestEnvelope.request.type === 'SessionEndedRequest') ||
                (handlerInput.requestEnvelope.request.type === 'IntentRequest' && request.intent.name === "AMAZON.StopIntent")) { // skill was stopped or timed out 
