...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        //} else if (attributes.drillStatus[attributes.drillStatusIndex].drill.completed) {
            // upsell the next 
        //    return makeUpsell('all righty. Yo completed the last drill.', getNextPurchasableProduct(attributes.purchasableProducts), handlerInput); // offer the first available purchaseable product. whhat if there are none left to buy???
        } else if (attributes.drillStatus[attributes.drillStatusIndex].drill.completed) {
            speechOutput = requestAttributes.t('WELCOME_BACK_DRILL_COMPLETE', drills[attributes.currentDrillIndex].packName) + qta;
        } else {
            // kick off with the drill & level the user was last on
            speechOutput = requestAttributes.t('WELCOME_BACK', drills[attributes.currentDrillIndex].packName, 
...
const YesIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        switch (attributes.skillState) {
            ...
            //    speechOutput = 'instant replay. ' + setupLevel(handlerInput);
            //    break;
...
function setupLevel(handlerInput) {
    ...
    if (! ISPHelp.isEntitled(attributes.entitledProducts, attributes.currentDrillRef)) {
        console.log('############################################### IM VERY SORRY BUT YOU ARE NOT ENTITLED');
    }
    ...
    attributes.currentDrillIndex = drills.map(function(e) { return e.ref; }).indexOf(attributes.currentDrillRef); // get the index position in the drills array
    ...
    //attributes.currentLevel = attributes.drillStatus[(attributes.drillStatus.map(function(e) { return e.drill.ref; }).indexOf(attributes.currentDrillRef))].drill.level;
    ...
    attributes.drillStatusIndex = attributes.drillStatus.map(function(e) { return e.drill.ref; }).indexOf(attributes.currentDrillRef); // get the index position in the drillStatus arra

    if (attributes.drillStatus[attributes.drillStatusIndex].drill.completed) {
        //return 'You have completed this one. want another?';
        return 'Would you like to replay it?';
    }
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (product.length === 0) { // User has aked for the free pack
                attributes.drillRef = drills[0].ref; // perfect intervals
                let qta = setupLevel(handlerInput);
                ...
                handlerInput.attributesManager.getPersistentAttributes()
                    .then((persistentAttributes) => {
                        ...
                        if (!persistentAttributes.launchCount) { // first ever launch
                            ...
                            persistentAttributes.currentDrillRef = 'perfect-intervals'; // all new ussers start with the free perfect intervals drill
