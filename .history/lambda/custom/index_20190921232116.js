...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (attributes.launchCount === 1) {
            ...
            speechOutput = '<break time="7s"/>' + requestAttributes.t('WELCOME_BACK_DRILL_COMPLETE', drills[attributes.currentDrillIndex].packName) + qta;
            
        } else {
            // kick off with the drill & level the user was last on
            speechOutput = '<break time="7s"/>' + requestAttributes.t('WELCOME_BACK', drills[attributes.currentDrillIndex].packName, 
...
function setupLevel(handlerInput) {
    ...
    if (attributes.drillStatus[attributes.drillStatusIndex].drill.completed) {
        ...
        return 'Would you like to replay this drill?';
    }
    ...
    handle(handlerInput) {
        ...
        return monetizationClient.getInSkillProducts(locale).then((res) => {
            ...
            if (ISPHelp.isEntitled(attributes.entitledProducts, attributes.currentDrillRef)) {
                ...
                // Give the user a different message here if the current drill has been completed 
                ...
                handlerInput.attributesManager.getPersistentAttributes()
                    .then((persistentAttributes) => {
                        ...
                        if (!persistentAttributes.launchCount) { // first ever launch
                            ...
                            persistentAttributes.currentDrillRef = freePackRef; // all new ussers start with the free perfect intervals drill
