...
exports.pickAKey = function (arrayIn) {
 // console.log(arrayIn);
  // Randomly pick one of the enharmonic equivalents for each test
  var arrayOut = arrayIn;

  for (i = 0, len = arrayIn.length; i < len; i++) {
    //for (j = 1, len2 = arrayIn[i].length; j < 3; j++) { // only array elements 1 & 2 can contain enharmonic equivalents
      a = arrayIn[i].split('/');
      if (a.length == 2) {
        // Array element contains enharmonic equivalent
        var enhKey = a[Math.floor((Math.random() * 2))]; // random number between 0 and 1
      } else {
        var enhKey = a[0];
      }
      arrayOut[i] = enhKey;
