...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        if (attributes.launchCount === 1) {
            ...
            speechOutput = '<break time="10s"/>' + requestAttributes.t('WELCOME_BACK_DRILL_COMPLETE', drills[attributes.currentDrillIndex].packName) + qta;
            
        } else {
            // kick off with the drill & level the user was last on
            speechOutput = '<break time="10s"/>' + requestAttributes.t('WELCOME_BACK', drills[attributes.currentDrillIndex].packName, 
