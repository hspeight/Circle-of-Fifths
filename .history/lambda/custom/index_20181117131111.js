...
const LaunchRequestHandler = {
    ...
    async handle(handlerInput) {
        ...
        const pAttributes = await attributesManager.getPersistentAttributes() || {};
        if (Object.keys(pAattributes).length === 0) {
            //attributes.endedSessionCount = 0;
            //attributes.gamesPlayed = 0;
            // user has not set the number of questions so give them 6
            pAttributes.numQuestionsThisUser = 6;
        }
        handlerInput.attributesManager.setSessionAttributes(pAttributes);
