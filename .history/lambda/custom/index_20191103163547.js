...
const S3Help = require('./helpers/S3Helper');
...
const AnswerHandler = {
    ...
    handle(handlerInput) {
        ...
        if (supportsAPL(handlerInput)) {
            ...
            if (attributes.skillState === states.ENDOFLEVEL || attributes.skillState === states.REPLAYLEVEL) {
                dataNextQuestion.bodyTemplate7Data.text.question = 'LEVEL COMPLETE'; //convert
            } else {
...
const PersistenceRequestInterceptor = {
    process(handlerInput) {
        if (handlerInput.requestEnvelope.session.new === true) {
            console.log('DRILLS '.repeat(8));
            console.log(drills);
            console.log('DRILLS '.repeat(8));
            drills = getMeThatTherDrillData(); // get the drill data from the S3 bucket every new session
            console.log('DRILLS '.repeat(8));
            console.log(drills);
            console.log('DRILLS '.repeat(8));
...
async function getMeThatTherDrillData() {

    console.log('start');
    //console.log(await S3Help.getDrilldataFromS3());
    const D = await S3Help.getDrilldataFromS3();
    return D;
    //console.log('done');
    //const drills = await S3Help.getDrilldataFromS3();
