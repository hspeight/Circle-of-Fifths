...
const AnswerHandler = {
    ...
    handle(handlerInput) {
        ...
        var str = handlerInput.requestEnvelope.request.intent.slots.ANSWER.value.toLowerCase().replace('.','');
...
const PurchaseHistoryHandler = {
	...
	async handle(handlerInput) {
        ...
        if (attributes.entitledProducts.length > 0) { // please convert to requestattributes
            speechOutput = `You have purchased the following packs: ${getSpeakableListOfProducts(attributes.entitledProducts)}. 
                                ${requestAttributes.t('RANDOM_YES_NO_QUESTION')}`;
            repromptOutput = `This is the list of products you have purchased: ${getSpeakableListOfProducts(entitledProducts)}. 
                            ${requestAttributes.t('RANDOM_YES_NO_QUESTION')}`;
        } else {
...
const BuyPackIntentHandler = {
  ...
  handle(handlerInput) {  
    ...
    return ms.getInSkillProducts(locale).then(function(res) {
      let product = res.inSkillProducts.filter(record => record.referenceName == "relative-keys"); // what about additional packs?
      ...
      return handlerInput.responseBuilder
        .addDirective({
          ...
          'payload': {
            'InSkillProduct': {
              'productId': product[0].productId // what about additional packs?
            }
          },
          'token': 'correlationToken'
        })
